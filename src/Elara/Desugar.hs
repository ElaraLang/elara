...
data PartialDeclaration
  ...
  | Immediate (Located Desugared.DeclarationBody)
  deriving (Show)
...
resolvePartialDeclaration ((JustLet sr e)) = pure (Located sr $ Desugared.DeclarationBody (Located sr (Desugared.Value e Nothing)))
resolvePartialDeclaration ((Both sr ty e)) = pure (Located sr $ Desugared.DeclarationBody (Located sr (Desugared.Value e (Just ty))))
    ...
    genPartial (Located wholeDeclRegion decl) =
      ...
      where
        ...
        genPartial'' (Frontend.TypeDeclaration vars typeDecl) = do
          ...
          pure (Immediate (Located bodyLoc (Desugared.DeclarationBody (Located bodyLoc decl'))))
