...
locatedPattern p = (\(Located sr node) -> Pattern (PatLoc sr) Nothing node) <$> located p

varPattern :: Parser FrontendPattern
varPattern = locatedPattern (PVar . tagLocated @VarNode . fmap NormalVarName <$> located varId)
