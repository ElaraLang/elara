{
  bounds = {
    elara = {
      HUnit = {
        lower = "1.6.0.0";
        upper = "1.7";
      };
      QuickCheck = {
        lower = "2.14.3";
        upper = "2.16";
      };
      aeson = {
        lower = "2.2.3.0";
        upper = "2.4";
      };
      algebraic-graphs = {
        lower = "0.0.1";
        upper = "0.8";
      };
      array = {
        lower = "0.5.8.0";
        upper = "0.6";
      };
      autodocodec = {
        lower = "0.2.3.0";
        upper = "0.7";
      };
      base = {
        lower = "4.21.1.0";
        upper = "4.22";
      };
      binary = {
        lower = "0.8.9.3";
        upper = "0.9";
      };
      bytestring = {
        lower = "0.12.2.0";
        upper = "0.13";
      };
      co-log-core = {
        lower = "0.3.2.6";
        upper = "0.4";
      };
      co-log-effectful = {
        lower = "0.0.0.1";
        upper = "0.1";
      };
      containers = {
        lower = "0.7";
        upper = "0.8";
      };
      dependent-hashmap = {
        lower = "0.1.0.0";
        upper = "0.2";
      };
      dependent-sum-template = {
        lower = "0.2.0.2";
        upper = "0.3";
      };
      diagnose = {
        lower = "1.8.2";
        upper = "2.6";
      };
      directory = {
        lower = "1.3.9.0";
        upper = "1.4";
      };
      effectful = {
        lower = "2.6.1.0";
        upper = "2.7";
      };
      effectful-core = {
        lower = "2.6.1.0";
        upper = "2.7";
      };
      effectful-plugin = {
        lower = "2.0.0.1";
        upper = "2.1";
      };
      effectful-th = {
        lower = "1.0.0.3";
        upper = "1.1";
      };
      elara = {
        lower = null;
        upper = null;
      };
      filepath = {
        lower = "1.5.4.0";
        upper = "1.6";
      };
      generic-optics = {
        lower = "2.2.1.0";
        upper = "2.4";
      };
      h2jvm = {
        lower = "0.1.0.0";
        upper = "0.2";
      };
      hashable = {
        lower = "1.5.1.0";
        upper = "1.6";
      };
      hedgehog = {
        lower = "1.5";
        upper = "1.6";
      };
      hspec-megaparsec = {
        lower = "2.2.1";
        upper = "2.3";
      };
      kind-generics-th = {
        lower = "0.2.3.3";
        upper = "0.3";
      };
      lens = {
        lower = "5.3.6";
        upper = "5.4";
      };
      lifted-base = {
        lower = "0.2.3.2";
        upper = "0.3";
      };
      matrix = {
        lower = "0.3.6.2";
        upper = "0.4";
      };
      megaparsec = {
        lower = "9.5.0";
        upper = "9.8";
      };
      mtl = {
        lower = "2.3.1";
        upper = "2.4";
      };
      neat-interpolation = {
        lower = "0.5.1.4";
        upper = "0.6";
      };
      opt-env-conf = {
        lower = "0.0.0.0";
        upper = "0.16";
      };
      optics = {
        lower = "0.4.2.1";
        upper = "0.5";
      };
      parser-combinators = {
        lower = "1.0.0";
        upper = "1.4";
      };
      pretty-simple = {
        lower = "3.2.1.0";
        upper = "4.2";
      };
      prettyprinter = {
        lower = "1.7.1";
        upper = "1.8";
      };
      prettyprinter-ansi-terminal = {
        lower = "1.1.2";
        upper = "1.2";
      };
      process = {
        lower = "1.6.26.1";
        upper = "1.7";
      };
      relude = {
        lower = "1.2.2.2";
        upper = "1.3";
      };
      safe-exceptions = {
        lower = "0.1.7.4";
        upper = "0.2";
      };
      some = {
        lower = "1.0.6";
        upper = "1.1";
      };
      stringsearch = {
        lower = "0.3.6.6";
        upper = "0.4";
      };
      sydtest = {
        lower = "0.12.0.0";
        upper = "0.29";
      };
      sydtest-hedgehog = {
        lower = "0.4.0.0";
        upper = "0.5";
      };
      template-haskell = {
        lower = "2.23.0.0";
        upper = "2.24";
      };
      terminal-size = {
        lower = "0.3.0";
        upper = "0.4";
      };
      text-metrics = {
        lower = "0.3.3";
        upper = "0.4";
      };
      time = {
        lower = "1.14";
        upper = "1.15";
      };
      utf8-string = {
        lower = "0.3.7";
        upper = "1.1";
      };
      witch = {
        lower = "1.3.1.0";
        upper = "1.4";
      };
    };
  };
  versions = {
    latest = {
      HUnit = "1.6.2.0";
      QuickCheck = "2.15.0.1";
      aeson = "2.3.1.0";
      algebraic-graphs = "0.7";
      array = "0.5.8.0";
      autodocodec = "0.6.0.0";
      base = "4.21.1.0";
      binary = "0.8.9.3";
      bytestring = "0.12.2.0";
      co-log-core = "0.3.2.7";
      co-log-effectful = "0.0.0.1";
      containers = "0.7";
      dependent-hashmap = "0.1.0.1";
      dependent-sum-template = "0.2.0.2";
      diagnose = "2.5.1";
      directory = "1.3.9.0";
      effectful = "2.6.1.0";
      effectful-core = "2.6.1.0";
      effectful-plugin = "2.0.0.1";
      effectful-th = "1.0.0.3";
      filepath = "1.5.4.0";
      generic-optics = "2.3.0.0";
      h2jvm = "0.1.0.0";
      hashable = "1.5.1.0";
      hedgehog = "1.5";
      hspec-megaparsec = "2.2.1";
      kind-generics-th = "0.2.3.3";
      lens = "5.3.6";
      lifted-base = "0.2.3.12";
      matrix = "0.3.6.4";
      megaparsec = "9.7.0";
      mtl = "2.3.1";
      neat-interpolation = "0.5.1.4";
      opt-env-conf = "0.15.0.2";
      optics = "0.4.2.1";
      parser-combinators = "1.3.1";
      pretty-simple = "4.1.4.0";
      prettyprinter = "1.7.1";
      prettyprinter-ansi-terminal = "1.1.4";
      process = "1.6.26.1";
      relude = "1.2.2.2";
      safe-exceptions = "0.1.7.4";
      some = "1.0.6";
      stringsearch = "0.3.6.6";
      sydtest = "0.28.0.0";
      sydtest-hedgehog = "0.4.0.0";
      template-haskell = "2.23.0.0";
      terminal-size = "0.3.4";
      text-metrics = "0.3.3";
      time = "1.14";
      utf8-string = "1.0.2";
      witch = "1.3.1.0";
    };
    lower = {
      HUnit = "1.6.0.0";
      QuickCheck = "2.14.3";
      aeson = "2.2.3.0";
      algebraic-graphs = "0.0.1";
      array = "0.5.8.0";
      autodocodec = "0.2.3.0";
      base = "4.21.1.0";
      binary = "0.8.9.3";
      bytestring = "0.12.2.0";
      co-log-core = "0.3.2.6";
      co-log-effectful = "0.0.0.1";
      containers = "0.7";
      dependent-hashmap = "0.1.0.0";
      dependent-sum-template = "0.2.0.2";
      diagnose = "1.8.2";
      directory = "1.3.9.0";
      effectful = "2.6.1.0";
      effectful-core = "2.6.1.0";
      effectful-plugin = "2.0.0.1";
      effectful-th = "1.0.0.3";
      filepath = "1.5.4.0";
      generic-optics = "2.2.1.0";
      h2jvm = "0.1.0.0";
      hashable = "1.5.1.0";
      hedgehog = "1.5";
      hspec-megaparsec = "2.2.1";
      kind-generics-th = "0.2.3.3";
      lens = "5.3.6";
      lifted-base = "0.2.3.2";
      matrix = "0.3.6.2";
      megaparsec = "9.5.0";
      mtl = "2.3.1";
      neat-interpolation = "0.5.1.4";
      opt-env-conf = "0.0.0.0";
      optics = "0.4.2.1";
      parser-combinators = "1.0.0";
      pretty-simple = "3.2.1.0";
      prettyprinter = "1.7.1";
      prettyprinter-ansi-terminal = "1.1.2";
      process = "1.6.26.1";
      relude = "1.2.2.2";
      safe-exceptions = "0.1.7.4";
      some = "1.0.6";
      stringsearch = "0.3.6.6";
      sydtest = "0.12.0.0";
      sydtest-hedgehog = "0.4.0.0";
      template-haskell = "2.23.0.0";
      terminal-size = "0.3.0";
      text-metrics = "0.3.3";
      time = "1.14";
      utf8-string = "0.3.7";
      witch = "1.3.1.0";
    };
  };
  initial = {
    latest = { };
    lower = {
      HUnit = "1.6.0.0";
      QuickCheck = "2.15.0.1";
      aeson = "2.3.0.0";
      algebraic-graphs = "0.7";
      array = "0.5.8.0";
      autodocodec = "0.6.0.0";
      binary = "0.8.9.3";
      bytestring = "0.12.2.0";
      co-log-core = "0.3.2.6";
      co-log-effectful = "0.0.0.1";
      containers = "0.7";
      dependent-hashmap = "0.1.0.0";
      dependent-sum-template = "0.2.0.2";
      diagnose = "2.0.0";
      directory = "1.3.9.0";
      effectful = "2.6.1.0";
      effectful-core = "2.6.1.0";
      effectful-plugin = "2.0.0.1";
      effectful-th = "1.0.0.3";
      filepath = "1.5.4.0";
      generic-optics = "2.3.0.0";
      hashable = "1.5.1.0";
      hedgehog = "1.5";
      hspec-megaparsec = "2.2.1";
      kind-generics-th = "0.2.3.3";
      lens = "5.3.6";
      lifted-base = "0.2.3.2";
      matrix = "0.3.6.2";
      megaparsec = "9.7.0";
      mtl = "2.3.1";
      neat-interpolation = "0.5.1.4";
      opt-env-conf = "0.15.0.0";
      optics = "0.4.2.1";
      parser-combinators = "1.3.0";
      pretty-simple = "4.1.0.0";
      prettyprinter = "1.7.1";
      prettyprinter-ansi-terminal = "1.1.2";
      process = "1.6.26.1";
      relude = "1.2.2.2";
      safe-exceptions = "0.1.7.4";
      some = "1.0.6";
      stringsearch = "0.3.6.6";
      sydtest = "0.23.0.0";
      sydtest-hedgehog = "0.4.0.0";
      terminal-size = "0.3.0";
      text-metrics = "0.3.3";
      time = "1.14";
      utf8-string = "1.0.1.1";
      witch = "1.3.1.0";
    };
  };
  overrides = {
    latest = {
      aeson = {
        version = "2.3.1.0";
        hash = "0s1jb8ymlp0qkmx3qjj03239yxbxy3sg74krv5yp9kg4pwylsrdx";
        repo = "hackage.haskell.org";
      };
      autodocodec = {
        version = "0.6.0.0";
        hash = "1vsxcb4vnjq180c264gvz3jxsbb1mdf2gzwkfh08ap29lz47wkxh";
        repo = "hackage.haskell.org";
      };
      autodocodec-nix = {
        version = "0.1.0.2";
        hash = "0xhih99mlqc2gj1b2zn58mdflx3dgc0a5248z5qs26jrjgq90mkk";
        repo = "hackage.haskell.org";
      };
      autodocodec-schema = {
        version = "0.2.0.2";
        hash = "0jripbj5zs7vq4ya7qyzz6zxa55vq2c974ki7y2njqb19dz6lyl6";
        repo = "hackage.haskell.org";
      };
      autodocodec-yaml = {
        version = "0.4.0.3";
        hash = "07x0y0ipfwkcjz12kflsbj84rnps24g2050qcdv1n59x0apaij5q";
        repo = "hackage.haskell.org";
      };
      co-log-core = {
        version = "0.3.2.7";
        hash = "1d7vi3a0aj6xxap34dzc6rajnypsjsw2pb8y6jia90psg5wnm27n";
        repo = "hackage.haskell.org";
      };
      co-log-effectful = {
        version = "0.0.0.1";
        hash = "1gysg3vw6wig43n9w6jhifsvyaniagmqv73gsydzacq502i8646q";
        repo = "hackage.haskell.org";
      };
      constraints-extras = {
        version = "0.4.0.2";
        hash = "150a742f67sjmaac4g40hzvkdnmnhsf2g49zn3jsh4p2brf9hvp5";
        repo = "hackage.haskell.org";
      };
      dependent-hashmap = {
        version = "0.1.0.1";
        hash = "10nyskpxwpa4fj71fv9fl0bb6vr786mfls2l9arfwbrdx539qlrf";
        repo = "hackage.haskell.org";
      };
      dependent-sum = {
        version = "0.7.2.0";
        hash = "1m85k11y97y7v6705bb6f9kpg6bcbm521p1p1812v0ybzkr2mg80";
        repo = "hackage.haskell.org";
      };
      diagnose = {
        version = "2.5.1";
        hash = "1r0yjl4f76zk2x59yh58470w1mxrk1d18ay22zzxzdblafrp7p74";
        repo = "hackage.haskell.org";
      };
      generic-lens-core = {
        version = "2.3.0.0";
        hash = "05im3y27lhjjy6hi0i85rlqsan510fmp63lqfwg18cnlzn0yvf81";
        repo = "hackage.haskell.org";
      };
      generic-optics = {
        version = "2.3.0.0";
        hash = "179ksdrrhj3m3ishgy62bxxm5dfa2flg58vxbqz8h895006wj46z";
        repo = "hackage.haskell.org";
      };
      genvalidity = {
        version = "1.1.1.0";
        hash = "09xwwzq1kfz37y1z81ddryp8kr84f1l1j5qj6b6qzn4kal4s215z";
        repo = "hackage.haskell.org";
      };
      genvalidity-containers = {
        version = "1.0.0.2";
        hash = "0c2wdj8bn6hdprbnwnyh74fhpp3gcnkbg2ngvylg6dg0sx9y02b6";
        repo = "hackage.haskell.org";
      };
      genvalidity-path = {
        version = "1.0.0.1";
        hash = "1fla1cjm7yrpdyj8gifl7kg4m9livc0liwqc30sf4q966g1jd7zs";
        repo = "hackage.haskell.org";
      };
      genvalidity-text = {
        version = "1.0.0.1";
        hash = "0kk7c4y2ymim2k62r5bzmsbqw6m0pr1hxvgf5bnkgy71lfr8pwg9";
        repo = "hackage.haskell.org";
      };
      opt-env-conf = {
        version = "0.15.0.2";
        hash = "181qrw8kgcj7lwwk99vgk036vwayx0cni0zxmr6fdg993d1i4rdr";
        repo = "hackage.haskell.org";
      };
      optparse-applicative = {
        version = "0.19.0.0";
        hash = "0cs8fqipakad38lvm75nz98hmvf881mgjhnc7icblxfzh92ay6kn";
        repo = "hackage.haskell.org";
      };
      path = {
        version = "0.9.6";
        hash = "16hgrkvd27c9vp5447d1dv3b3fi0fv0jfig10h2j37mzk4850wg8";
        repo = "hackage.haskell.org";
      };
      path-io = {
        version = "1.8.2";
        hash = "063ma7gzqr5c6s8a1yv72jgll3xdajvgclbc8w0ddmqgcrb62x2k";
        repo = "hackage.haskell.org";
      };
      pretty-simple = {
        version = "4.1.4.0";
        hash = "06sfilnrmr8cx5m2ajjn4y3fyx6gxibyqc73mkw22wfk8pivll0j";
        repo = "hackage.haskell.org";
      };
      prettyprinter-ansi-terminal = {
        version = "1.1.4";
        hash = "0kyl47b83rs2kbifam2wimqv1zz80qkfwznkmq5qwlhr05gxrz58";
        repo = "hackage.haskell.org";
      };
      sydtest = {
        version = "0.28.0.0";
        hash = "105frfhkjhmq6ivwyvwfkf5p0jc1b3cwydpqsx8ppy10dmg7dnqa";
        repo = "hackage.haskell.org";
      };
      sydtest-hedgehog = {
        version = "0.4.0.0";
        hash = "06an6p0mhdxm4rhqnhd92f2k248b70fs67xxg74v0v3g7l27iy1s";
        repo = "hackage.haskell.org";
      };
      sydtest-mutation-runtime = {
        version = "0.1.1.0";
        hash = "1ifxp27kr1grrmzps565p8snjyw2jnbq1c129qdrvvn02g39dw5f";
        repo = "hackage.haskell.org";
      };
      validity-aeson = {
        version = "0.2.0.5";
        hash = "0wirg6wjzpc37073dia3c82i2svihn2bbh31kxwqxjl6zqw5a1ia";
        repo = "hackage.haskell.org";
      };
      validity-path = {
        version = "0.4.0.1";
        hash = "16m76x5p9ihjp5ndhawf79hj26bpy8j9inpxg36jziy5dvc608hf";
        repo = "hackage.haskell.org";
      };
      yaml = {
        version = "0.11.11.2";
        hash = "0d9xlsbqf0gwjmqh4r51jnafndan6bf4zj5nklzl6b8x1wnjm7l3";
        repo = "hackage.haskell.org";
      };
    };
    lower = {
      Diff = {
        version = "1.0.2";
        hash = "0s0bxg8qygbkjg6jf6hi689s616qfq0kg1mraar2229zvx54673x";
        repo = "hackage.haskell.org";
      };
      HUnit = {
        version = "1.6.0.0";
        hash = "011qaixcy05w0jazw5x4qir16j6rvdax6ayf0fjmhf9hlbii1pk8";
        repo = "hackage.haskell.org";
      };
      QuickCheck = {
        version = "2.14.3";
        hash = "18451rdmih1jkrsrckdcix71zqihc4h2caic7qzizxjg4hqpapji";
        repo = "hackage.haskell.org";
      };
      aeson = {
        version = "2.2.3.0";
        hash = "1a9a0z6ljbck5scwkk9r9p04y9avn9vja3n063lyqgn2v1vjb1sp";
        repo = "hackage.haskell.org";
      };
      algebraic-graphs = {
        version = "0.0.1";
        hash = "0p4palm7ddbra9f1sga68h26q7gwj2vgxnyw6frisjdkz2hj166w";
        repo = "hackage.haskell.org";
      };
      autodocodec = {
        version = "0.2.3.0";
        hash = "1mvsv7kn9p80b7d511wh1w9nc7dvlb7s28gcgk3bdqxw9qrh5irv";
        repo = "hackage.haskell.org";
      };
      autodocodec-schema = {
        version = "0.1.0.4";
        hash = "0i8yac5viiddn6hq5pvj11nf8z1p18lvl07rfhp2msc0a1ykr2ck";
        repo = "hackage.haskell.org";
      };
      autodocodec-yaml = {
        version = "0.3.0.0";
        hash = "02sd2c1jjlf30kx611bnc3jahs856w0nsszk4c7xh9xlinbnmyfs";
        repo = "hackage.haskell.org";
      };
      cabal-doctest = {
        version = "1.0.12";
        hash = "094mvqgh9bhx5v9xanzkhcm8pcxzmkaa68lr3bqpjzkdxydx81nk";
        repo = "hackage.haskell.org";
      };
      concurrent-output = {
        version = "1.10.21";
        hash = "1w87rrf337s8wc4z3dkh2mk990003jsk18ry5yawv4465k4yvamw";
        repo = "hackage.haskell.org";
      };
      constraints-extras = {
        version = "0.4.0.2";
        hash = "150a742f67sjmaac4g40hzvkdnmnhsf2g49zn3jsh4p2brf9hvp5";
        repo = "hackage.haskell.org";
      };
      dependent-hashmap = {
        version = "0.1.0.0";
        hash = "1x5vysxbnd0h09j5api8mn8ss33sm2dcp4zp4p0qjyl6zwkaadam";
        repo = "hackage.haskell.org";
      };
      dependent-sum = {
        version = "0.7.2.0";
        hash = "1m85k11y97y7v6705bb6f9kpg6bcbm521p1p1812v0ybzkr2mg80";
        repo = "hackage.haskell.org";
      };
      diagnose = {
        version = "1.8.2";
        hash = "0g97ys6d5yr30y3jagkq7r0w93sh27kf2ghflsyich88b5i1n79m";
        repo = "hackage.haskell.org";
      };
      envparse = {
        version = "0.6.0";
        hash = "0z55z3vyz64akc4y0fz02kb440yg5w8qhbq59b6zqr62vkisc8i2";
        repo = "hackage.haskell.org";
      };
      generic-lens-core = {
        version = "2.2.1.0";
        hash = "0innx8ndljssasw9f1fnhwhgjv3smn57kir895d7fbj4r0k8w11s";
        repo = "hackage.haskell.org";
      };
      generic-optics = {
        version = "2.2.1.0";
        hash = "08p14izcf954rbwhr43qkbxdcfcc84b910788iyrwh7fwznffasz";
        repo = "hackage.haskell.org";
      };
      generically = {
        version = "0.1.1";
        hash = "1658df4jbhzgpsqqgxp71bgry7ak5q0bjlrw6x551g54vvcydbxz";
        repo = "hackage.haskell.org";
      };
      hashable = {
        version = "1.5.1.0";
        hash = "02mk0fxkqrx11qffs7jl231bfflz10vyx5s5xqn8y7ayyndmb6db";
        repo = "hackage.haskell.org";
        revision = true;
      };
      hedgehog = {
        version = "1.5";
        hash = "1hz8xrg5p6vplvcj8c7pgidqnwqjmqahs9dla50nqpbcbdh932ll";
        repo = "hackage.haskell.org";
      };
      hsc2hs = {
        version = "0.68.10";
        hash = "0wdg17kicnp6qbgynha216vihx7nnsglvhn5c089dqpa14hg35zw";
        repo = "hackage.haskell.org";
      };
      hspec-expectations = {
        version = "0.8.4";
        hash = "0dasnpk5d9s0mazbvhwq6dh6jd67fhbw0s34pinha4mbmb9qcnij";
        repo = "hackage.haskell.org";
      };
      hspec-megaparsec = {
        version = "2.2.1";
        hash = "0w925cqfynq7yarc7w28q7f7y4r5cf9y9l7xlxzknigiabm8mwlf";
        repo = "hackage.haskell.org";
      };
      lifted-async = {
        version = "0.10.2.7";
        hash = "0cgzs8sfr3l7ah5nnscpp50v5mmvc4hqf02zdi4h344dbbha10fy";
        repo = "hackage.haskell.org";
      };
      lifted-base = {
        version = "0.2.3.2";
        hash = "10wx30rrmg1gg315r88mv5hr72aw4gm37ddhnhh715q8l59gh21s";
        repo = "hackage.haskell.org";
      };
      matrix = {
        version = "0.3.6.2";
        hash = "11mfpivrwy69dlalbshgjnkn28rya0l486bf2gc31bs0j4cnva0s";
        repo = "hackage.haskell.org";
      };
      megaparsec = {
        version = "9.5.0";
        hash = "1y3szhj2a2ias4k7v7q2dcxvxw242vbl99ih7szlvwpaj67ixlzy";
        repo = "hackage.haskell.org";
      };
      neat-interpolation = {
        version = "0.5.1.4";
        hash = "005rjq6wa7q01cvm0yzisqah8qn9p4a5y538xglas0iap5w77cwn";
        repo = "hackage.haskell.org";
      };
      opt-env-conf = {
        version = "0.0.0.0";
        hash = "1h4b51nkba936xiayn85aa162bmzkiccj19vkv22kgsr0l3s13l6";
        repo = "hackage.haskell.org";
      };
      optparse-applicative = {
        version = "0.19.0.0";
        hash = "0cs8fqipakad38lvm75nz98hmvf881mgjhnc7icblxfzh92ay6kn";
        repo = "hackage.haskell.org";
      };
      parser-combinators = {
        version = "1.0.0";
        hash = "0kaag6fz103iz2cncr5zhy671dp266g4phkz8kq52w23wkmh8fqs";
        repo = "hackage.haskell.org";
      };
      path = {
        version = "0.9.6";
        hash = "16hgrkvd27c9vp5447d1dv3b3fi0fv0jfig10h2j37mzk4850wg8";
        repo = "hackage.haskell.org";
      };
      path-io = {
        version = "1.8.2";
        hash = "063ma7gzqr5c6s8a1yv72jgll3xdajvgclbc8w0ddmqgcrb62x2k";
        repo = "hackage.haskell.org";
      };
      pretty-simple = {
        version = "3.2.1.0";
        hash = "1hh8f82brrasx7nql15z12kgm6fly5bvvqaz2hy5lliwylfrgywj";
        repo = "hackage.haskell.org";
      };
      prettyprinter-ansi-terminal = {
        version = "1.1.2";
        hash = "0lwcqndppw3jc55rlnn6sp76zmjx2yzl21g9jhg27k2rdnjwd7md";
        repo = "hackage.haskell.org";
      };
      quickcheck-io = {
        version = "0.2.0";
        hash = "1flw5h455z6vr0ghv1i4z1rc033jp1hcv4ci1jnqryd6pyh95ny3";
        repo = "hackage.haskell.org";
      };
      sydtest = {
        version = "0.12.0.0";
        hash = "1in0xi9cajzwxfv703rz8b5ramliqpmyfwrci5c05h7y17adw2wy";
        repo = "hackage.haskell.org";
      };
      sydtest-hedgehog = {
        version = "0.4.0.0";
        hash = "06an6p0mhdxm4rhqnhd92f2k248b70fs67xxg74v0v3g7l27iy1s";
        repo = "hackage.haskell.org";
      };
      terminal-size = {
        version = "0.3.0";
        hash = "0rc3z6nf8189zk014nna568sg2hpxmyqlv6ha1y0fhpw9m8872bk";
        repo = "hackage.haskell.org";
      };
      time-compat = {
        version = "1.9.9";
        hash = "02yq6qc9fbawpxkypaf4nm9vidfv5vvgidxyj4r3dxa4lb29jd2p";
        repo = "hackage.haskell.org";
        revision = true;
      };
      utf8-string = {
        version = "0.3.7";
        hash = "1bzwj4d29a0xrbccvay8iw3k9qirs634jgk1s1ma3fwrrzs9iyc3";
        repo = "hackage.haskell.org";
      };
      validity-aeson = {
        version = "0.2.0.5";
        hash = "0wirg6wjzpc37073dia3c82i2svihn2bbh31kxwqxjl6zqw5a1ia";
        repo = "hackage.haskell.org";
      };
      witch = {
        version = "1.3.1.0";
        hash = "1n848h6dsyflpn7hn72pjnz3j06fg7i6gay1w6namjy597miyg1l";
        repo = "hackage.haskell.org";
        revision = true;
      };
      yaml = {
        version = "0.11.11.2";
        hash = "0d9xlsbqf0gwjmqh4r51jnafndan6bf4zj5nklzl6b8x1wnjm7l3";
        repo = "hackage.haskell.org";
      };
    };
  };
  solver = {
    latest = { };
    lower = { };
  };
  packages = { };
  resolving = false;
}
